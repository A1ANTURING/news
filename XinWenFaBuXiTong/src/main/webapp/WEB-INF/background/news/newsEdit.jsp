...
<div class="data_list backMain">
	...
	<div class="data_content">
		<form action="${pageContext.request.contextPath }/admin/news/edit/${news.newsid }.action" method="post" enctype="multipart/form-data" onsubmit="return checkForm()">
			<table cellpadding="5" width="100%">
				<tr>
					...
					<td>
						<input type="text" value="${news.title }" id="title" name="title" class="input-xxlarge"/>
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" value="${news.author }" id="author" name="author" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<select id="typeId" name="typeId">
							...
							<c:forEach var="newsType" items="${newsTypeList }">
								<c:if test="${news.tNewstype.newstypeid == newsType.newstypeid}">
									<option selected="selected" value="${newsType.newstypeid }">${newsType.typename }</option>
								</c:if>
								<c:if test="${news.tNewstype.newstypeid != newsType.newstypeid}">
				...
				<tr>
					...
					<td>
						<label class="checkbox inline">
							<c:if test="${news.ishead == 1}">
								<input type="checkbox" checked="checked" id="isHead" name="isHead" value="1">头条
							</c:if>
						  	<c:if test="${news.ishead != 1}">
						...
						<label class="checkbox inline">
							<c:if test="${news.isimage == 1}">
								 <input type="checkbox" checked="checked" id="isImage" name="isImage" onclick="checkChange()" value="1">幻灯
							</c:if>
						  	<c:if test="${news.isimage != 1}">
						...
						<label class="checkbox inline">
							<c:if test="${news.ishot == 1}">
								 <input type="checkbox" checked="checked" id="isHot" name="isHot" value="1">热点
							</c:if>
						  	<c:if test="${news.ishot != 1}">
				...
				<c:if test="${news.isimage == 1 }">
					<tr id="hdtp">
						...
						<td>
							<input type="file" id="picFile" name="file" />
							<img src="${pageContext.request.contextPath }/userImage/${news.imagename}">
						</td>
					</tr>
				</c:if>
